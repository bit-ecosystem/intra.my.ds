...
class FeaturesOverview extends Widget
{
    ...
    {
        $post = Course::query()->first();
        $order = Course::query()->first();
        $expense = Course::query()->first();
        $product = Course::query()->first();
        $project = Course::query()->first();
        $employee = Course::query()->first();
    ...
    protected function tablesCategory(): array
    {
        return [
            ...
            'color' => 'blue',
            'features' => [
                ['name' => 'Searchable & sortable', 'description' => 'Full-text search with sortable column headers', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Image columns', 'description' => 'Thumbnails from Spatie Media Library', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Column summarizers', 'description' => 'Scroll to the table footer to see sum totals for price and shipping', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Inline editing', 'description' => 'Click a status cell to change it inline', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Table grouping', 'description' => 'Toggle grouping using the group icon in table header', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Live polling', 'description' => 'Table data auto-refreshes every 30 seconds in the background', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Toggleable columns', 'description' => 'Click the column toggle icon in the table header', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Color columns', 'description' => 'Hidden by default — enable "Team color" via the column toggle icon to see swatches', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Column layouts', 'description' => 'Custom multi-row layouts with split and stack', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Drag-and-drop reordering', 'description' => 'Click the reorder toggle in the table header, then drag rows', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Copyable columns', 'description' => 'Click any email cell to copy the value to your clipboard', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
            ],
        ];
    }
    ...
    protected function filtersCategory(): array
    {
        return [
            ...
            'color' => 'violet',
            'features' => [
                ['name' => 'Query builder', 'description' => 'Click the filter icon above the table', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Select filters', 'description' => 'Open the filter panel to see select-based filters for type, department & more', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Ternary filter', 'description' => 'Open the filter panel to find the three-state "Is active" toggle', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Trashed filter', 'description' => 'Open the filter panel to toggle soft-deleted record visibility', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
                ['name' => 'Filters above content', 'description' => 'Click the collapsed filter bar above the table to expand it', 'url' => CourseResource::getUrl('index'), 'resource' => 'Courses'],
            ],
        ];
    }
