...
trait HasAttachableRoles
{
    ...
    protected function normalizeRolesInput(null|array|string|int $rolesInput): array
    {
        ...
        foreach ((array) $rolesInput as $item) {
            ...
            if (is_array($item)) {
                $name = \Illuminate\Support\Arr::get($item, 'name');
                $id = \Illuminate\Support\Arr::get($item, 'id');
                $teamId = $this->normalizeTeamId(\Illuminate\Support\Arr::get($item, 'team_id'));
    ...
    public function scopeVisibleToUserInOrgUnit(Builder $builder, $user, \App\Models\Core\OrgUnit $orgUnit): Builder
    {
        if (! $user || ! $orgUnit) {
            return $builder->whereRaw('1=0');
        }
    ...
    public function scopeVisibleToUserByUserTeams(Builder $builder, $user, \App\Models\Core\OrgUnit $orgUnit): Builder
    {
        if (! $user || ! $orgUnit) {
            return $builder->whereRaw('1=0');
        }
